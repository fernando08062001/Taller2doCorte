...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    threshold_margin = 0.1*threshold
    power_difference = abs(power - threshold)
    if power < 0.9* threshold:
        return 'LOW'
    if  power_difference <= threshold_margin:
        #if power is within plus or minus 10% of the threshold
        return 'NORMAL'
    return 'DANGER'